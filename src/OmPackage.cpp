...
///
OmPackage::OmPackage() :
  _modChan(nullptr), _type(0), _ident(), _hash(0), _core(), _version(), _name(),
...
///  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
///
OmPackage::OmPackage(OmModChan* pModChan) :
  _modChan(pModChan), _type(0), _ident(), _hash(0), _core(), _version(), _name(),
  _src(), _srcDir(), _srcTime(0), _srcItemLs(), _depLs(), _bck(), _bckDir(),
  _bckItemLs(), _ovrLs(), _category(), _desc(), _descTime(0), _thumb(), _thumbTime(0),
  _error()
{
...
void OmPackage::footprint(vector<OmPkgItem>& footprint) const
{
  ...
  for(size_t i = 0; i < this->_srcItemLs.size(); ++i) {
    ...
    Om_concatPaths(path, this->_modChan->_dstDir, this->_srcItemLs[i].path);
...
void OmPackage::log(unsigned level, const wstring& head, const wstring& detail)
{
  if(this->_modChan != nullptr) {
    ...
    wstring log_str = L"ModChan("; log_str.append(this->_modChan->title());
    log_str.append(L"):: "); log_str.append(head);

    this->_modChan->log(level, log_str, detail);
  }
}
...
bool OmPackage::_backup(int zipLvl, Om_progressCb progress_cb, void* user_ptr)
{
  ...
  if(is_zip) {
    ...
    this->_bck = this->_modChan->_bckDir; //< Mod Channel Backup location
    this->_bck += L"\\";
  ...
  this->_modChan->pkgFindOverlaps(ovlap_list, this);
  ...
  for(size_t i = 0, z = 0; i < this->_srcItemLs.size(); ++i) {
    ...
    Om_concatPaths(app_file, this->_modChan->_dstDir, this->_srcItemLs[i].path);
    ...
    if(Om_pathExists(app_file)) {
      ...
      if(this->_srcItemLs[i].type == PKGITEM_TYPE_F) { //< this is a file
        ...
        if(this->_modChan->bckItemExists(bck_item.path, PKGITEM_DEST_DEL)) {
          #ifdef DEBUG
...
bool OmPackage::_restore(Om_progressCb progress_cb, void* user_ptr, bool undo)
{
  ...
  if(Om_isFileZip(this->_bck)) {
    ...
    for(size_t i = 0; i < this->_bckItemLs.size(); ++i) {
      ...
      if(this->_bckItemLs[i].dest == PKGITEM_DEST_CPY) {
        ...
        Om_concatPaths(app_file, this->_modChan->_dstDir, this->_bckItemLs[i].path);
        // extract from zip
  ...
  while(n--) {
    ...
    if(this->_bckItemLs[n].dest == PKGITEM_DEST_DEL) {
      ...
      Om_concatPaths(del_file, this->_modChan->_dstDir, this->_bckItemLs[n].path);
